...
class KeyboardControlNode:
    ...
    def __init__(self):
        ...
        self.accel_rate = rospy.get_param('~accel_rate', 0.1)
        self.steering_rate = rospy.get_param('~steering_rate', 0.3)
        self.control_topic = rospy.get_param('~control_topic', '/user_servo')#'/Control')
                
        # Create a publisher for the robot's velocity commands
        self.control_pub = rospy.Publisher(self.control_topic, ServoMsg, queue_size=10)
